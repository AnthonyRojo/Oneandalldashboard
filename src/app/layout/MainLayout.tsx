...
const NOTIF_NAV: Record<string, string> = {
  task: "/app/tasks", project: "/app/dashboard", member: "/app/team",
  announcement: "/app/announcements", event: "/app/calendar",
  comment: "/app/announcements", submit: "/app/tasks", team: "/app/dashboard",
};
...
function LayoutContent() {
  ...
  const [lastReadNotif, setLastReadNotif] = useState(() => localStorage.getItem("lastReadNotif") || "");
  ...
  const activeMessages = activeGroupId === null
    ? currentMessages.filter((m) => !m.groupId)
    : currentMessages.filter((m) => m.groupId === activeGroupId);
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      setNewTeamError(err.message || "Failed.");
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto">
          ...
          <div className="space-y-1">
            {NAV_ITEMS.map(({ label, icon: Icon, path }) => {
              ...
              return (
                <button key={path} onClick={() => { navigate(path); setSidebarOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left" style={{ background: active ? "rgba(245,158,11,0.15)" : "transparent", color: active ? "#f59e0b" : "#9ca3af" }}>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-1">
              {BOTTOM_ITEMS.map(({ label, icon: Icon, path }) => {
                const active = location.pathname === path;
                return (
                  <button key={path} onClick={() => { navigate(path); setSidebarOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left" style={{ background: active ? "rgba(245,158,11,0.15)" : "transparent", color: active ? "#f59e0b" : "#9ca3af" }}>
                    <Icon className="w-4 h-4 flex-shrink-0" />
                    <span style={{ fontSize: "0.875rem", fontWeight: active ? 600 : 400 }}>{label}</span>
                  </button>
                );
                    ...
                    {notifications.length === 0 ? (
                      ...
                      return (
                        <button key={n.id} className="w-full text-left px-4 py-3 flex items-start gap-3 border-b last:border-b-0 transition-all"
                          ...
                          onClick={() => { navigate(NOTIF_NAV[n.type] || "/app/dashboard"); setNotifOpen(false); }}
            ...
            <div className="relative ml-1">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl border bg-white shadow-xl z-50" style={{ borderColor: "#f0f0ea" }}>
                  ...
                  <div className="p-2">
                    <button onClick={() => { navigate("/app/settings"); setProfileOpen(false); }} className="w-full text-left px-3 py-2 rounded-xl transition-colors" style={{ fontSize: "0.875rem", color: "#374151" }}
